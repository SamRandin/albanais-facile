...
import { lessons } from '@/data/lessons-elvish'
import ElvishLessonCard from './ElvishLessonCard'

export default function ElvishHomePage() {
  const totalWords = lessons.reduce((sum, l) => sum + l.words.length, 0)

  return (
    <>
      {/* Hero */}
      <section className="bg-gradient-to-br from-emerald-500 via-emerald-600 to-emerald-700 text-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-20 sm:py-28">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm rounded-full px-4 py-1.5 text-sm font-semibold mb-6">
              <span>🧝</span>
              <span>Quenya — Langue des Hauts-Elfes</span>
            </div>
            ...
            <p className="text-lg sm:text-xl text-white/90 mb-8 leading-relaxed">
              La langue des Hauts-Elfes de Tolkien — le Quenya — est une
              langue construite complète avec une grammaire et un vocabulaire
              documentés. Partez à la découverte de la Terre du Milieu par
              les mots.
            </p>
            <div className="flex flex-wrap gap-3">
              <Link
                href="/elfique#lecons"
                className="inline-flex items-center gap-2 bg-white text-emerald-700 font-bold px-6 py-3 rounded-full shadow hover:bg-emerald-50 transition-colors"
              >
      ...
      <section className="bg-white border-b border-emerald-100">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 grid grid-cols-1 sm:grid-cols-3 gap-6">
          {[
            { icon: '🗂️', label: 'Leçons thématiques', desc: 'Vocabulaire organisé par univers et situations' },
      ...
      <section id="lecons" className="max-w-5xl mx-auto px-4 sm:px-6 py-14">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {lessons.map((lesson) => (
            <ElvishLessonCard key={lesson.id} lesson={lesson} />
          ))}
        </div>
      </section>

      {/* CTA section */}
      <section className="bg-gradient-to-r from-emerald-500 to-emerald-700 text-white mt-4 mb-10 mx-4 sm:mx-6 rounded-3xl max-w-5xl lg:mx-auto">
        <div className="px-8 py-10 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            ...
            <p className="text-white/80 text-sm">
              Commencez par les salutations — &ldquo;Aiya mellon nín !&rdquo;
            </p>
          </div>
          <Link
            href="/elfique/salutations"
            className="flex-shrink-0 bg-white text-emerald-700 font-bold px-6 py-3 rounded-full shadow hover:bg-emerald-50 transition-colors"
          >
            Première leçon →
          </Link>
        </div>
